...
$connStr = 
    if ($env:SECRETS_CONTEXT -and $(
        $workflowSecrets = ConvertFrom-Json $env:SECRETS_CONTEXT;$workflowSecrets.GIT_LOGGER_CONNECTION_STRING
    )) {
        $workflowSecrets.GIT_LOGGER_CONNECTION_STRING
    } elseif ($(
        $secureSettingValue = Get-Secret -Name GitloggerConnectionString -AsPlainText; $secureSettingValue
    )) {
        $secureSettingValue
    }
...
$repoRestUrl = $gitLoggerPushUrl + '/' + ($gitRemoteUrl.RemoteUrl -replace '^(?>https?|git|ssh)://')
